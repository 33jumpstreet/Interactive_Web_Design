$(document).ready(function(){
  ...
  $("html, bidy").animate({"scrollTop":widSec},2000);
  ...
  $("article h2").on("click", function(e){
    ...
    // 변수 index에 현재 클릭한 h2의 부모의 article의 순서값을 저장
    var index = $(this).parent().index();
    // 변수 src에 현재 클릭한 h의 자식 요소인 a의 href값을 저장
    ...
    // 모든 article p img의 이미지 속성값을 지움
    $("article p img").attr({"src":""});
    // 현재 클릭된 article의 형제요소 중 p를 찾아 다시 그 자식인 img의 주소값을 변수 src값으로 변경
    $(this).siblings("p").children("img").attr({"src":src});
    // 문서의 스크롤 위치값을 변수 posA 값으로 변경
